...
def test_is_field_setter_classifies_by_kind():
    ...
    assert is_field_setter(pagetype.command(FIELDS, "setLabel"))       # SET_SCALAR
    assert is_field_setter(pagetype.command(FIELDS, "setBody"))        # SET_PROSE
    assert is_field_setter(pagetype.command(FIELDS, "addItem"))        # ADD_ELEMENT
    assert is_field_setter(pagetype.command(life, "askQuestion"))      # ADD_ELEMENT (element-FSM list add)
    assert not is_field_setter(pagetype.command(FIELDS, "removeItem"))     # REMOVE_ELEMENT
    assert not is_field_setter(pagetype.command(FIELDS, "reorderItem"))    # REORDER_ELEMENT
    assert not is_field_setter(pagetype.command(FIELDS, "flagItem"))       # SET_ELEMENT_FIELD
    assert not is_field_setter(pagetype.command(FIELDS, "addLink"))        # ADD_LINK
    assert not is_field_setter(pagetype.command(FIELDS, "setTitle"))       # SET_TITLE
    assert not is_field_setter(pagetype.command(life, "answerQuestion"))   # ELEMENT_TRANSITION
    assert not is_field_setter(pagetype.command(life, "beginPlanning"))    # TRANSITION
    # A page-level blocks add is now an ordinary field setter: one add per field, so it names
    # exactly one command in `do`.
    assert is_field_setter(pagetype.command(blocks, "addBody"))             # ADD_BLOCK, page-level
    assert not is_field_setter(pagetype.command(blocks, "removeBlock"))     # REMOVE_BLOCK
    ...
    assert not is_field_setter(pagetype.command(element_blocks, "addItemDetail"))
...
def test_field_setter_edges_use_the_instruction_key_not_description():
    ...
    assert pagetype.command(seal, "setOverview").description == "set the overview"
...
def test_element_scoped_block_adds_are_withheld_from_the_fields_edge():
    ...
    assert edges[0]["instruction"] == pagetype.field_spec(ELEMENT_BLOCKS, "items", "items").description
