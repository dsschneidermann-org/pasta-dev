...
**Architecture = pure core + stateful shell.** Pure (no I/O, unit-tested): `src/{model,fsm,commands,serialize,describe}.py` and the `src/pagetypes/` package. Shell (integration-tested): `src/{store,server}.py`.
